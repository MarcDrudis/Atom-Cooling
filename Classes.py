...
class MonteCarlo:
    def __init__(self,atom,N=100,timesteps=100,deltaL=None,sigmar=5e-4,sigmav=0.81,dt=20e-6,Omega12=5e5,Magnetic_gradient=np.array([5,5,5]),waist= 2e-3):
        ...
        self.temperature_measure=[('module',None),('temperature',None),('mean',None),('multiply',1e6)]
...
class Optimizer:
    ...
    def dic(self):
        d={'method':self.method,'Parameters':self.parameters,'Efficiency':self.efficiency,'Velocity':self.velocity, 'Temperature':self.temperature}
        return d
    ...
    def firstGuess(self):
        first_heuristic=np.linalg.norm(self.monty_opt.V,axis=1).mean() * self.monty_opt.atom.k - self.monty_opt.mag[0] * self.monty_opt.waist
